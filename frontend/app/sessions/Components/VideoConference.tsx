...
const VideoF: React.FC<VideoFProps> = ({
  peer,
  userId,
  email,
  isPinned,
  ...
  const handleStream = (stream: any) => {
    if (videoRef.current) {
      ...
      const hasVideoOn =
        ...
        videoStreams.some(
          (track: any) => track.enabled && track.readyState == "live"
        );
  ...
  return (
    <div className={`${styles.videoContainer} relative h-full w-full`}>
      ...
      <video
        ...
        hidden={videoOffUsers.has(userId)}
      />

      <div className={styles.videoLabel}>
        {connected && audioMutedUsers.has(userId) ? (
          <MicOff />
        ) : (
          connected && !audioMutedUsers.has(userId) && <Mic />
        )}
        ...
        if (participant.type === "me") {
          return (
            <div
              ...
              } relative h-full w-full rounded-md overflow-hidden ${getParticipantClass(
                "me"
              )}
                transition-colors duration-1500 ease-out
                ${
                  amSpeaking && !isMuted
                    ? "border-3 border-cyan-600"
          ...
          return (
            <div
              key={peerData.peerId}
              className={`rounded-md flex items-center justify-center ${getParticipantClass(
                peerData.peerId
              )}
                transition-colors duration-1500 ease-out
                ${
                  speakingUsers.has(peerData.peerId)
                    ? "border-3 border-cyan-600"
                    : "border-3 border-transparent"
                }`}
            >
