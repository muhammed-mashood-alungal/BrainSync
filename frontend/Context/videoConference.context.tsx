...
export const VideoCallProvider = ({ roomId, children }: { roomId: string; children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const setupDetection = async () => {
      try {
        cleanupFunction = await detectSpeaking(myStreamRef.current!, socketRef.current?.id as string)
      } catch (error) {
  ...
  const detectSpeaking = async (stream: MediaStream, peerId: string) => {
    ...
    if (audioContext.audioWorklet === undefined) {
      console.warn("AudioWorklet not supported in this browser. Falling back to analyzer-only approach");
    ...
    try {
      ...
      const audioProcessor = new AudioWorkletNode(audioContext, 'audio-level-processor');
