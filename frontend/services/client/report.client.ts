import { reportInsances } from "@/axios/instance.axios";
...
export const reportService = {
  ...
  getAllReports: async (status: string, skip: number, limit: number) => {
    try {
      const response = await reportInsances.get(
        `/all-reports?status=${status}&skip=${skip}&limit=${limit}`
      );
  ...
  resolve: async (reportId: string) => {
    try {
      const response = await reportInsances.put(
