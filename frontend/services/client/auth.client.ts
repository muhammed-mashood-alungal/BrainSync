import { authInstance } from "@/axios/instance.axios";
...
export const AuthServices = {
  ...
  resendOtp: async (
    ...
    try {
      ...
      const err = error as AxiosError<{ error: string }>;
      const errorMessage =
        err.response?.data?.error || "Resending Otp failed. Please try again.";
      throw new Error(errorMessage);
    }
  },
  authMe: async (): Promise<{
    id: string;
    email: string;
    role: string;
  ...
  googleAuth: async (): Promise<void> => {
    try {
      ...
      const err = error as AxiosError<{ error: string }>;
      const errorMessage =
        err.response?.data?.error || "Google Auth failed. Please try again.";
      throw new Error(errorMessage);
    }
  },
  forgotPassword: async (
    ...
    try {
      ...
      const errorMessage =
        ...
        "Sending Reset Link failed. Please try again.";
      throw new Error(errorMessage);
    }
  },
  ...
  resetPassword: async (
    token: string,
    password: string
  ): Promise<{ status: number; mesasge: string }> => {
    try {
      ...
      const err = error as AxiosError<{ error: string }>;
      const errorMessage =
        err.response?.data?.error || "Reset Password failed. Please try again.";
      throw new Error(errorMessage);
    }
  },
};
