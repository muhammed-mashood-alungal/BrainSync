...
export class NoteService implements INoteService {
    ...
    async saveNoteService(sessionCode: string, userId: string): Promise<{ pdfFileId: string }> {
        ...
        await this._noteRepository.createNote(session?._id as Types.ObjectId, userId, pdfFileId , session?.sessionName as string)
    ...
    async myNotes(userId: unknown, query: string , skip : unknown , limit : unknown): Promise<{notes : INoteModel[] ,count : number}> {
        const {notes , count} =  await this._noteRepository.myNotes(userId as Types.ObjectId , query , skip as number, limit as number)
        return {notes , count}
    }
}