...
export class GroupServices implements IGroupService {
    ...
    async addToGroup(groupId: unknown, members: unknown[]): Promise<void> {
        await this._groupRepository.addToGroup(groupId as Types.ObjectId, members as Types.ObjectId[])
    }

    async leftFromGroup(groupId: unknown, userId: unknown): Promise<void> {
        await this._groupRepository.leftGroup(groupId as Types.ObjectId, userId as Types.ObjectId)
    }
    ...
    async groupData(id: unknown): Promise<IGroupModel | null> {
        const result = await this._groupRepository.getGroupData(id as Types.ObjectId)
        if (!result) {
            throw createHttpsError(HttpStatus.NOT_FOUND, HttpResponse.RESOURCE_NOT_FOUND)
        }
        return result
    }
