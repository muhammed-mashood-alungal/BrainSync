...
export class NotificationController implements INotificationController{
    ...
    async getUserNotifications(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const userId = req.user
            const notifications = await this._notificationServices.getUserNotifications(userId)
            res.status(HttpStatus.OK).json(successResponse(HttpResponse.OK , {notifications}))
        } catch (error) {
            next(error)
        }
    }
