...
export class ReportRepository extends BaseRepository<IReportModel> implements IReportRepository {
    ...
    async getAllReportsAndCount(status: string, skip : number , limit : number): Promise<{reports : IReportModel[] , count : number}> {
        let find: any = {}
        if (status != "All") {
            find.status = status
        }
        
        const reports =  await this.model.find(find).populate("reportedBy").populate('sessionId').skip(skip).limit(limit)
