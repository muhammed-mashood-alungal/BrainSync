...
export class WhiteboardRepository implements IWhiteboardRepository {
    async saveWhiteboard(roomId: string, whiteboardData: CanvasData): Promise<void> {
        const whiteboardRef = ref(firebaseDB, `whiteboards/${roomId}/${whiteboardData.slideIndex}`)
    ...
    listenToBoardChanges(roomId: string, callback: (data: CanvasData | null) => void) : Unsubscribe {
        ...
        const unsubscribe = onValue(whiteboardRef, (snapshot : any) => {
            console.log('Something changed on room '+ roomId)
